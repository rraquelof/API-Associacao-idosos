...
export const createIdosoSchema = z.object({
    ...
    rg: z.string().regex(rgRegex, "Formato RG inválido. Formato esperado: x.xxx.xxx").optional(),
    sus: z.string().regex(susRegex, "Formato SUS inválido. Cartão SUS deve conter 15 dígitos").optional(),
    dataNascimento: z.string().refine((date) => !isNaN(Date.parse(date)), { message: "Data de nascimento inválida" }).optional(),
    sexo: z.enum(['masculino', 'feminino'], "Sexo deve ser 'masculino' ou 'feminino'").optional(),
    nacionalidade: z.string().min(3, "Nacionalidade deve ter pelo menos 3 caracteres").optional(),
    naturalidade: z.string().min(3, "Naturalidade deve ter pelo menos 3 caracteres").optional(),
